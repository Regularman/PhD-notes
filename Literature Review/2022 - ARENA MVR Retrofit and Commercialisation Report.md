...
- Compressor is designed with a bypass system so failure of one compressor is okay for the system
	...
	- Anti-surge valves are also required for startups. Low flow compressors can be started up using their variable speed drives until they reach maximum speed.
- One of the difficulty of the project is selecting a compressor that can operate over a significant volume range as they allow for one compressor to be out of circuit. (Caused additional 3% power consumption and 5% increase in capital cost)
	- ~={red}Note that the first 4 compressors in the MVR train can participate in ESS services through override switches to respond to voltage droops. There is another ARENA project that looks at the ability of MVR to provide MVR services.=~
...
- This risk can be mitigated using anti-surge valves. This is where compressor discharge is recirculated to the compressor inlet to allow operations with a low inlet flow. Controlled with a PLC.
...
MR for high temperature aluminium processing is also not proven. Requires steam at higher temperature and higher pressure. The duty is technically feasible but the technology will need to be developed.
- This might be achieved by smaller compressors on the final stages of the compressor train.
...
Look at SWIS WOSP double bubble, which look at grid constraints in the event of high electrification. The Whole of System Plan (WOSP) looks at market dispatch and resource planning model to look at different scenarios in the SWIS.

Report only thinks about operational savings.
